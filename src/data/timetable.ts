...
export const timetable: DaySchedule[] = [
...

  {
    ...
    periods: [
      ...
      {
        lectureNo: 6,
        subject: 'OOPS Lab (TPP)',
        shortSubject: 'OOPS LAB(TPP)',
        faculty: 'Mr. Pradeep',
        room: '301',
        startTime: '13:50',
        endTime: '15:40',
...
export function buildScheduleItems(periods: Period[]): ScheduleItem[] {
  ...
  for (let i = 0; i < periods.length; i++) {
    const p = periods[i]
    // Insert short break if gap between 10:50 and 11:00
    if (i > 0 && periods[i - 1].endTime === '10:50' && p.startTime === '11:00') {
      result.push({ kind: 'break', label: 'Short Break', startTime: '10:50', endTime: '11:00' })
    }
